...
class BaseModel(nn.Module):
    ...
    def __init__(self, num_classes=len(le.classes_)):
        ...
        self.classifier = nn.Linear(2560, num_classes) #기본 출력크기 1,000으로 정의
...
def train(model, optimizer, train_loader, val_loader, scheduler, device):
    ...
    best_score = 0
    best_model = None
    best_model_path = './best_model3.pth'  # 모델을 저장할 경로
    ...
    for epoch in range(1, CFG['EPOCHS']+1):
        ...
        if best_score < _val_score:
            ...
            best_model = model
            torch.save(best_model.state_dict(), best_model_path)  # 성능이 좋아질 때마다 모델 가중치 저장
            print(f'Model saved at {best_model_path} with F1 score {best_score:.5f}')
